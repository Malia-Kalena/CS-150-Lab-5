...
from train_model import load_and_preprocess_data, train_svm, get_decision_function, calculate_accuracy
...
def get_dataset(dataset_name):
    if dataset_name == "student-math":
        return load_and_preprocess_data("assets/student-mat.csv")
    elif dataset_name == "student-por":
        return load_and_preprocess_data("assets/student-por.csv")
    else:
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.H2(
                            ...
                            children=[
                                html.A(
                                    "SVM Classifier: Predicting Student Success in Math and Portuguese",
                                ...
                                drc.Card(
                                    ...
                                    children=[
                                        drc.NamedDropdown(
                                            ...
                                            options=[
                                                {"label": "Student Math", "value": "student-math"},
                                                {"label": "Student Portuguese", "value": "student-por"},
                                            ],
                                            clearable=False,
                                            searchable=False,
                                            value="student-math",
                                ...
                                drc.Card(
                                    ...
                                    children=[
                                        ...
                                        drc.FormattedSlider(
                                            ...
                                            value=1,
                                            step=2,
                                            marks={i: str(i) for i in range(1, 10, 2)},
                                        ),
                                        drc.NamedSlider(
                                            name="Degree",
...
def reset_threshold_center(n_clicks, figure):
    if n_clicks:
        try:
            ...
            if isinstance(Z.flat[0], dict):
                raise ValueError("Z contains dicts, not numeric values.")

            value = -Z.min() / (Z.max() - Z.min())
        except Exception as e:
            print("Error in reset_threshold_center:", e)
...
def update_svm_graph(
    ...
    # Data Pre-processing
    X_scaled, X_train, X_test, y_train, y_test = get_dataset(dataset)
    ...
    # Train SVM
    clf = train_svm(X_train, y_train, kernel, degree, C, gamma, flag)
    accuracy = calculate_accuracy(clf, X_test, y_test)
    Z = get_decision_function(clf, np.c_[xx.ravel(), yy.ravel()])    # decision boundary
